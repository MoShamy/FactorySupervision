...
cap = cv2.VideoCapture("Bottles_tracking.mp4")
...
while cap.isOpened():
    ...
    for x1, y1, x2, y2 in detections:
        ...
        for pcx, pcy in prev_centroids:
            if pcx < line_x and cx >= line_x:
                ...
                obj_count = obj_count + 1 if last_cross_time - start_time < 60 else 0
                last_cross_time = time.time()
    ...
    cv2.putText(frame, f"Objects per min: {obj_count}", (30, 90),
                cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 3)
